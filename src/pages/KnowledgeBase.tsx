...
import { Search, BookmarkPlus, ChevronRight, Eye, ThumbsUp, Play, BookOpen, Bookmark } from 'lucide-react';
...
const KnowledgeBase = () => {
  ...
  const categories = [
    '推荐', '减重科普', '饮食营养', '运动健身', '慢病管理', '心理健康', '睡眠改善'
  ];
  ...
  return (
    <div className="flex flex-col min-h-screen bg-black text-black">
      ...
      {/* Header */}
      <div className="flex justify-between items-center px-5 pt-10 pb-4 bg-black text-white">
      ...
      {/* Content */}
      <div className="flex-1 bg-white overflow-y-auto rounded-t-3xl px-5 pt-6 pb-24">
        {/* Search Bar */}
        ...
        <div className="flex overflow-x-auto no-scrollbar mb-5 pb-2">
          {categories.map((category) => (
            <div 
              key={category}
              className={`category-pill ${activeCategory === category ? 'category-pill-active' : 'category-pill-inactive'} mr-2`}
