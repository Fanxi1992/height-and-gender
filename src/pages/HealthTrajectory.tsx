...
const initialDiseaseData = [
  { id: 1, name: "二型糖尿病", initialRisk: 0.73, currentRisk: 0.73, category: "代谢疾病", sensitivity: 0.08, ageSensitivity: 0.04 },
  { id: 2, name: "高血压", initialRisk: 0.61, currentRisk: 0.61, category: "心血管疾病", sensitivity: 0.06, ageSensitivity: 0.05 },
  { id: 3, name: "肾结石", initialRisk: 0.55, currentRisk: 0.55, category: "泌尿系统疾病", sensitivity: 0.04, ageSensitivity: 0.02 },
  { id: 4, name: "高血糖", initialRisk: 0.43, currentRisk: 0.43, category: "代谢疾病", sensitivity: 0.07, ageSensitivity: 0.03 },
  { id: 5, name: "肠易激综合征", initialRisk: 0.31, currentRisk: 0.31, category: "消化系统疾病", sensitivity: 0.02, ageSensitivity: 0.015 },
  { id: 6, name: "浅表性胃炎", initialRisk: 0.28, currentRisk: 0.28, category: "消化系统疾病", sensitivity: 0.03, ageSensitivity: 0.02 },
  { id: 7, name: "偏头痛", initialRisk: 0.16, currentRisk: 0.16, category: "神经系统疾病", sensitivity: 0.015, ageSensitivity: 0.01 },
  { id: 8, name: "前列腺炎", initialRisk: 0.11, currentRisk: 0.11, category: "泌尿系统疾病", sensitivity: 0.01, ageSensitivity: 0.02 },
  { id: 9, name: "胆囊炎", initialRisk: 0.04, currentRisk: 0.04, category: "消化系统疾病", sensitivity: 0.005, ageSensitivity: 0.01 },
  { id: 10, name: "肝硬化", initialRisk: 0.02, currentRisk: 0.02, category: "消化系统疾病", sensitivity: 0.002, ageSensitivity: 0.008 },
];
...
const HealthTrajectory: React.FC = () => {
  ...
  const [currentAge, setCurrentAge] = useState(40); // 假设初始年龄
  const [initialAge] = useState(40);
  const [diseaseData, setDiseaseData] = useState([...initialDiseaseData]);
  const [ageBasedDiseaseData, setAgeBasedDiseaseData] = useState([...initialDiseaseData]);
  ...
  // 更新基于年龄的疾病风险
  useEffect(() => {
    const ageDifference = currentAge - initialAge;
    const updatedData = ageBasedDiseaseData.map(disease => {
      // 根据年龄变化和疾病年龄敏感度计算新的风险率
      const riskChange = ageDifference * disease.ageSensitivity;
      const newRisk = Math.max(disease.initialRisk + riskChange, 0.01); // 保证风险不小于0.01%
      return { ...disease, currentRisk: parseFloat(newRisk.toFixed(2)) };
    });
    
    setAgeBasedDiseaseData(updatedData);
  ...
  useEffect(() => {
    ...
    const width = ageChartRef.current.clientWidth;
    const margin = { top: 20, right: 30, bottom: 40, left: 50 };
    const chartWidth = width - margin.left - margin.right;
    const chartHeight = height - margin.top - margin.bottom;
    ...
    // 创建比例尺
    const x = d3.scaleBand()
      .domain(ageBasedDiseaseData.map(d => d.name))
      .range([0, chartWidth])
      .padding(0.3);

    const y = d3.scaleLinear()
      .domain([0, Math.max(...ageBasedDiseaseData.map(d => Math.max(d.initialRisk, d.currentRisk))) * 1.1])
      .range([chartHeight, 0]);
  ...
  const handleAgeChange = (change: number) => {
    // 限制年龄范围在20到80岁
    const newAge = Math.max(Math.min(currentAge + change, 80), 20);
  ...
  // 计算基于年龄的总风险变化百分比
  const calculateAgeRiskChange = () => {
    let initialTotal = 0;
    let currentTotal = 0;
    
    ageBasedDiseaseData.forEach(disease => {
      initialTotal += disease.initialRisk;
      currentTotal += disease.currentRisk;
    });
    ...
    const changePercent = ((currentTotal - initialTotal) / initialTotal) * 100;
  ...
  return (
    <div className="page-container bg-black">
      ...
      <div className="mt-10 px-4 flex justify-between items-center">
        <BackButton />
        <h1 className="text-center text-xl font-medium"></h1>
      ...
      {/* 年龄对疾病风险的影响模块 */}
      <div className="mx-4 mt-5 bg-gradient-to-b from-gray-900 to-black rounded-3xl p-6 shadow-lg border border-gray-800">
        <h2 className="text-xl font-bold mb-4 text-center text-white flex items-center justify-center">
        ...
        {/* 年龄调节器 */}
        <div className="w-full mb-5 bg-gray-800 rounded-xl p-4 flex flex-col items-center">
          <p className="text-gray-300 text-sm mb-2">调整年龄查看疾病风险变化</p>
          ...
          <div className="flex items-center justify-between w-full">
            ...
            <div className="text-center">
              <div className="text-3xl font-bold text-white">{currentAge} 岁</div>
              <div className="text-sm text-gray-400">
                {currentAge !== initialAge 
                  ? `${currentAge > initialAge ? '增加' : '减少'} ${Math.abs(initialAge - currentAge)} 岁` 
                  : '初始年龄'}
              </div>
            </div>
          ...
          <div className="mt-4 w-full bg-gray-700 h-1 rounded-full">
            <div 
              className="bg-gradient-to-r from-green-500 to-purple-500 h-1 rounded-full"
              style={{ width: `${((currentAge - 20) / 60) * 100}%` }}
            ></div>
          </div>
        </div>
        ...
        {/* 风险变化统计 */}
        {currentAge !== initialAge && (
          <div className="w-full mb-4 bg-gradient-to-r from-purple-900 to-blue-900 rounded-xl p-4 animate-fade-in">
            <div className="flex justify-between items-center">
              <p className="text-white font-medium">总体疾病风险变化：</p>
              <p className={`text-2xl font-bold ${Number(calculateAgeRiskChange()) > 0 ? 'text-red-400' : 'text-green-400'}`}>
                {Number(calculateAgeRiskChange()) > 0 ? '+' : ''}{calculateAgeRiskChange()}%
              </p>
            </div>
          </div>
        )}
      ...
      {/* 健康故事 */}
      {showStory && (
        <div className="mx-4 mt-2 bg-green-100 rounded-3xl p-5 shadow-lg animate-scale-in">
          <div className="flex">
            <div className="w-10 h-10 rounded-full bg-green-600 flex items-center justify-center flex-shrink-0">
              <Heart className="text-white" size={16} />
            </div>
            <div className="ml-3">
              <h3 className="text-md font-semibold text-green-900">真实案例</h3>
              <p className="text-green-800 text-sm mt-1">{healthStories[storyIndex]}</p>
            </div>
          </div>
        </div>
      )}
